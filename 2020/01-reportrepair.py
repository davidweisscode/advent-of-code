...
# Approach 2: Check if a complement to number pairs exist, which has 2020 as the triple sum, return product

for i in entries:
    for j in entries:
        if (2020 - i -j) in entries:
            print(i, j, 2020 - i - j, i + j + 2020 - i - j, i * j * (2020 - i - j))

# Approach 3: Sort list, increase three pointers in such a way that the sum changes the least
...
# while True:
#     print(i, j, k, entries[i], entries[j], entries[k], (entries[i] + entries[j] + entries[k]))
#     if (entries[i] + entries[j] + entries[k]) == 2020:
#         print(f"The three numbers {entries[i]}, {entries[j]}, and {entries[k]} add up to 2020.")
#         print(f"Their product is {entries[i] * entries[j] + entries[k]}")
#         break
#     elif (entries[i] + entries[j] + entries[k]) > 2020:
#         # minimal decrease
#         if (k - j) > 1:
#             k -= 1
#             continue
